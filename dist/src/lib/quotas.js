...
exports.consumeReservation = consumeReservation;
exports.refundReservation = refundReservation;
exports.isCircuitTripped = isCircuitTripped;
exports.tripCircuitFor = tripCircuitFor;
exports.adjustUserUsage = adjustUserUsage;
...
async function getQuotaStatus(params) {
    const planId = defaultPlanForRole(params.role);
    const limits = pricing_1.PLAN_LIMITS[planId];
    await ensureSchema();
    const client = getPool();
    if (!client) {
    ...
    const res = await client.query('SELECT tokens_used, usd_used FROM user_usage WHERE user_id = $1 AND month = $2', [params.userId, params.month]);
...
async function enforceQuota(params) {
    ...
    const cost = (0, pricing_1.estimateCostUSD)({
        model: params.model,
    ...
    try {
        const res = await client.query(`INSERT INTO user_usage (user_id, month, tokens_used, usd_used)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id, month) DO UPDATE
       SET tokens_used = user_usage.tokens_used + EXCLUDED.tokens_used,
           usd_used = user_usage.usd_used + EXCLUDED.usd_used,
           updated_at = NOW()
       RETURNING tokens_used, usd_used`, [params.userId, month, tokens, cost.usd]);
        ...
        // record daily rollup
        try {
            await client.query(`INSERT INTO user_usage_daily (user_id, day, tokens_used, usd_used)
         VALUES ($1, $2, $3, $4)
         ...
         SET tokens_used = user_usage_daily.tokens_used + EXCLUDED.tokens_used,
             ...
             updated_at = NOW()`, [params.userId, new Date().toISOString().slice(0, 10), tokens, cost.usd]);
        }
        catch (err) {
            // non-fatal
        }
        ...
        try {
            ...
            const sum = Number(((_a = res.rows[0]) === null || _a === void 0 ? void 0 : _a.sum) || 0);
            if (sum >= threshold) {
                tripCircuitFor(pauseMs);
                try {
                    await client.query(`INSERT INTO circuit_state (id, tripped_until, threshold_usd, paused_ms, updated_at)
             VALUES (1, NOW() + ($1 || ' milliseconds')::interval, $2, $3, NOW())
             ON CONFLICT (id) DO UPDATE SET tripped_until = NOW() + ($1 || ' milliseconds')::interval, threshold_usd = $2, paused_ms = $3, updated_at = NOW()`, [pauseMs, threshold, pauseMs]);
                }
                catch { }
            }
        }
...
async function reserveQuota(params) {
    ...
    try {
        await client.query(`INSERT INTO quota_reservations (task_id, user_id, month, tokens_reserved, usd_reserved) VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT (task_id) DO UPDATE SET tokens_reserved = EXCLUDED.tokens_reserved, usd_reserved = EXCLUDED.usd_reserved, refunded = FALSE, consumed = FALSE, created_at = NOW()`, [params.taskId, params.userId, params.month, params.tokens, params.usd]);
        return true;
    }
    catch (err) {
        return false;
...
async function consumeReservation(taskId) {
    const client = getPool();
    if (!client)
        return true;
    try {
...
async function refundReservation(taskId) {
    await ensureSchema();
    const client = getPool();
    if (!client) {
    ...
    try {
        ...
        await client.query(`UPDATE user_usage SET tokens_used = GREATEST(tokens_used - $1,0), usd_used = GREATEST(usd_used - $2,0), updated_at = NOW() WHERE user_id = $3 AND month = $4`, [tokens, usd, row.user_id, row.month]);
        await client.query(`UPDATE user_usage_daily SET tokens_used = GREATEST(tokens_used - $1,0), usd_used = GREATEST(usd_used - $2,0), updated_at = NOW() WHERE user_id = $3 AND day = $4`, [tokens, usd, row.user_id, row.month + '-01']);
...
async function adjustUserUsage(params) {
    ...
    try {
        ...
        await client.query(`INSERT INTO user_usage (user_id, month, tokens_used, usd_used)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (user_id, month) DO UPDATE
       SET tokens_used = GREATEST(user_usage.tokens_used + $3, 0),
           usd_used = GREATEST(user_usage.usd_used + $4, 0),
           updated_at = NOW()`, [params.userId, params.month, tokensDelta, usdDelta]);
        if (params.day) {
            await client.query(`INSERT INTO user_usage_daily (user_id, day, tokens_used, usd_used)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (user_id, day) DO UPDATE
         SET tokens_used = GREATEST(user_usage_daily.tokens_used + $3, 0),
             ...
             updated_at = NOW()`, [params.userId, params.day, tokensDelta, usdDelta]);
        }
        await client.query('COMMIT');
        return true;
    }
...
async function getCircuitStatus() {
    ...
    try {
        ...
        const row = res.rows[0];
        const until = (row === null || row === void 0 ? void 0 : row.tripped_until) ? new Date(row.tripped_until).getTime() : circuitTrippedUntil || null;
        return { tripped: Date.now() < (until || 0), until, thresholdUsd: Number((row === null || row === void 0 ? void 0 : row.threshold_usd) || process.env.QUOTA_CIRCUIT_THRESHOLD_USD || 1000), pausedMs: Number((row === null || row === void 0 ? void 0 : row.paused_ms) || process.env.QUOTA_CIRCUIT_PAUSE_MS || 600000) };
    }
    catch {
        return { tripped: isCircuitTripped(), until: circuitTrippedUntil, thresholdUsd: Number(process.env.QUOTA_CIRCUIT_THRESHOLD_USD || 1000), pausedMs: Number(process.env.QUOTA_CIRCUIT_PAUSE_MS || 600000) };
    }
